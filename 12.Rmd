...
`timec4`:Patients’ symptoms do appear to improve significantly over time in the CART=0 group over the group of the 4 DVs. 

`CART0`: The effect of treatment is not significantly greater for the anxiety outcomes (HADSA and ASI) than for the mood outcomes (HADSD, PANASneg).

`AnxietyIndex`: The difference between the anxiety and depression when everything else is 0. At time point 4, for the CART group, the different between anxiety and depression scores. Anxiety has slightly higher scores than depression scores since positive (pretending it’s significant).

`timec4*AnxietyIndex`: The amount anxiety versus depression changes over time for the CART group. The effect of our IV and the DV depends on our moderator. The effect of time on our outcomes depend on whether it’s anxiety or depression. If it was significant. 

`CART0*AnxietyIndex`: The amount of anxiety versus depression at the last assessment between conditions is not significant.

`CART0*AnxietyIndex*timec4`: The difference between groups and change over time does not depend on whether it’s an anxiety or depressive symptom. 
