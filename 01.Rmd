...
/VARSTOCASES 
  /MAKE YBOC FROM YBOCpre YBOC4 YBOC16 YBOC52
  /MAKE BDI FROM BDIpre BDI4 BDI16 BDI52
  /INDEX=index1(5)
...
```
* STEP 2. Run Simple MLM with DV as YBOC and covariates as index1 and condition, and with
random effect for the intercept only. 
...
```{r, echo = FALSE}
knitr::include_graphics("https://raw.githubusercontent.com/carolinelee78/mlmbookdown/main/_figures/IMG4.png")
```
...
model1 <- lmer(YBOC ~ Index1 + condition + Index1*condition + (1 | Index1), data = DF1)
summary(model1)
```
